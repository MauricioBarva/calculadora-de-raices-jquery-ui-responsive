...
window.addEventListener("load", () => {
    ...
    $(document).ready(function() {
        ...
        var plot = $('#plot-biseccion');
        var pixel_ratio = window.devicePixelRatio;
        $(window).resize(function() {
            if (pixel_ratio != window.devicePixelRatio) {
                pixel_ratio = window.devicePixelRatio
                if (pixel_ratio < 1 || pixel_ratio > 1) {
                    plot.css('margin-right', '0px');
                } else if (pixel_ratio == 1) {
                    plot.css('margin-right', '28px');
                }
            }
        });
        $("#tabular").tabs();
        ...
        function biseccion() {
            ...
            iteracion.value = contador;
            errorEsperado.value = es;
            errorAproximado.value = ea;
            raizAproximada.value = xr;
            valorReemplazando.value = fxr;
